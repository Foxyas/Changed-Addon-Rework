...
public class TransfurTotemItem extends Item {
	...
	public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level level, @NotNull Player player, @NotNull InteractionHand hand) {
		...
		boolean isValidUse = player.getOffhandItem().is(stack.getItem())
				&& (player.getMainHandItem().is(stack.getItem()) || player.getMainHandItem().isEmpty());
	...
	public void inventoryTick(@NotNull ItemStack itemstack, @NotNull Level level, @NotNull Entity entity, int slot, boolean selected) {
		...
		if(!player.getMainHandItem().is(itemstack.getItem()) && !player.getOffhandItem().is(itemstack.getItem())) return;

		if(player.getCooldowns().isOnCooldown(itemstack.getItem()) || !ProcessTransfur.isPlayerTransfurred(player)
				|| !ProcessTransfur.getPlayerTransfurVariant(player).is(ChangedTransfurVariants.LATEX_BENIGN_WOLF.get())) return;
	...
	private static void tryLinkForm(Level level, Player player, ItemStack itemstack){
		...
		if (ChangedAddonServerConfiguration.ACCEPT_ALL_VARIANTS.get() || latexForm.startsWith("changed:form")) {
			linkForm(level, player, itemstack, latexForm);
			...
			linkForm(level, player, itemstack, "changed:form_light_latex_wolf");
		}
	}

	private static void linkForm(Level level, Player player, ItemStack stack, String form) {
		stack.getOrCreateTag().putString("form", form);

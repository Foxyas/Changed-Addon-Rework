...
//        entityTypes.removeIf(entityType -> !ForgeRegistries.ENTITY_TYPES.getKey(entityType).getPath().contains("protogen"));
    ...
    public static void registerAttributes(@NotNull EntityAttributeCreationEvent event) {
        ...
        for (Pair<Supplier<EntityType<? extends LivingEntity>>, Supplier<AttributeSupplier.Builder>> entitiesWithDefaultAttribute : EntitiesWithDefaultAttributes) {
            Supplier<EntityType<? extends LivingEntity>> entityTypeSupplier = entitiesWithDefaultAttribute.getFirst();
            if (DefaultAttributes.hasSupplier(entityTypeSupplier.get())) {
                continue;
            } else {
                Supplier<AttributeSupplier.Builder> second = entitiesWithDefaultAttribute.getSecond();
                event.put(entityTypeSupplier.get(), second.get().build());
            }
        }

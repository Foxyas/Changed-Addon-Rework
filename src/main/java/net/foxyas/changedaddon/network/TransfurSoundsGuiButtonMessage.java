	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		switch (buttonID){
			case 0, 1, 2, 3, 4, 5, 6 -> {
				...
				if(triple.getMiddle().test(entity)) playSound(level, entity, ForgeRegistries.SOUND_EVENTS.getValue(triple.getLeft()), vars, triple.getRight());
			}
			case 7 -> {
				if(!vars.act_cooldown) break;
			...
			case 8 -> {
				...
				} else commands.performCommand(entity.createCommandSourceStack().withSuppressedOutput().withPermission(4), "playsound changed:monster2 hostile @a ~ ~ ~ 5 1");

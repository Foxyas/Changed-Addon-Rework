	...
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (world.isClientSide()) {
			...
			Msg = "" + (new TranslatableComponent("changedaddon.warn.close_fight_to_keep_consciousness").getString()).replace("(KEY)",
					GLFW.glfwGetKeyName(ChangedAddonModKeyMappings.OPENGRABESCAPEGUI.getKey().getValue(), GLFW.glfwGetKeyScancode(ChangedAddonModKeyMappings.OPENGRABESCAPEGUI.getKey().getValue())));

...
public class IflatexentityProcedure {
    public static boolean execute(LevelAccessor world, double x, double y, double z, Entity entity) {
        ...
        Entity Entitya = null;
        Entity target = null;
        Entitya = world.getEntitiesOfClass(LivingEntity.class, AABB.ofSize(new Vec3(x, y, z), 64, 64, 64), e -> true).stream().sorted(new Object() {
        ...
        if ((entity instanceof net.ltxprogrammer.changed.entity.ChangedEntity || target instanceof Monster) && target.getType().is(TagKey.create(Registry.ENTITY_TYPE_REGISTRY, new ResourceLocation("changed:latexes")))) {
            return true;
        } else return target instanceof Player && (entity.getCapability(ChangedAddonModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ChangedAddonModVariables.PlayerVariables())).transfur

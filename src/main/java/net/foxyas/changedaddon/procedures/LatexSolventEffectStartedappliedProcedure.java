...
public class LatexSolventEffectStartedappliedProcedure {
	public static void execute(Entity entity) {
		...
		if (entity instanceof LivingEntity && ((LivingEntity) entity).getAttribute(ChangedAddonModAttributes.LATEXRESISTANCE.get()) != null) {
			...
			if (!(((LivingEntity) entity).getAttribute(ChangedAddonModAttributes.LATEXRESISTANCE.get()).hasModifier(LatexSolvent)))
				((LivingEntity) entity).getAttribute(ChangedAddonModAttributes.LATEXRESISTANCE.get()).addTransientModifier(LatexSolvent);
		}
	}
}
